...
import { Option } from '@md-modules/shared/components/form/select/index';
// themes
...
export const getSelectStyles = ({ isFocused, isError, isDisabled }: SelectStylesProps): StylesConfig<Option, any> => {
  return {
    ...
    control: (provided) => ({
      ...
      border: `${isFocused ? 2 : 1}px ${isDisabled ? 'dotted' : 'solid'} ${isError ? colors.red400 : colors.yellow400}`,
      ':hover': {
        border: `${isFocused ? 2 : 1}px solid ${isError ? colors.red400 : colors.yellow400}`
      }
    }),
    multiValue: (provided) => ({
      ...provided,
